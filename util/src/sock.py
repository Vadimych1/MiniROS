            ...
            while True:
                ...
                try:
                    match Datatypes(datatype):
                        case Datatypes.SEND_AUTH:
                            ...
                            self.servers[CREDENTIALS] = Connection(
                                name=CREDENTIALS, fields={}, socket=conn
                            )
                        ...
                        case Datatypes.GET:
                            ...
                            if (
                                node_name not in self.servers
                                or field_name not in self.servers[node_name].fields
                            ):
                                self.send(
                                    conn,
                                    bytes(
                        ...
                        case Datatypes.POST:
                            ...
                            for subscriber in (
                                self.servers[CREDENTIALS].fields[field_name].subscribers
                            ):
                                self.send(
                                    self.servers[subscriber].socket,
                                    bytes(
                                        [
                                            Datatypes.SEND_GET.value,
                                            len(CREDENTIALS),
                            ...
                            self.send(
                                conn,
                                bytes([Datatypes.SEND_POST.value, Status.OK.value]),
                                addr,
                            )
                    ...
                    self.send(
                        conn,
                        bytes(
                            [Datatypes.ERROR.value, Errortypes.METHOD_NOT_FOUND.value]
                        ),
                        addr,
                    )
    ...
    async def _check(self, CREDENTIALS: bytes, additional: str | None = None):
        if CREDENTIALS is None:
            raise ConnectionError(
                f"node hasn`t sended valid credentials {('(' + str(additional) + ')') if additional is not None else ''}"
            )
...
class AsyncDistributedClient(SockClient):
